...
export default function PrestadorScreen({ navigation }) {
  ...
  const aceitarChamado = (id) => {
    update(ref(db, `chamados/${id}`), { status: "Em andamento" });
  };

  return (
    <View style={styles.container}>
      <Text style={styles.titulo}>Área do Prestador</Text>
        ...
        renderItem={({ item }) => (
          <View style={styles.item}>
            ...
            {item.status === "Pendente" && (
              <TouchableOpacity onPress={() => aceitarChamado(item.id)}>
                <Text style={styles.botaoAceitar}>Aceitar Chamado</Text>
              </TouchableOpacity>
            )}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  botaoAceitar: { color: "#4cd137", marginTop: 10 },
